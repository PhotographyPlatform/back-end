...
async function handleGetAllPostUser(req, res, next) {
  try {
    ...
    const theRecord = await modules.newUserCOll.readAll(
      id,
      modules.post,
      modules.comment,
      modules.like
...
async function handleGetAllPostData(req, res, next) {
  try {
    const id = req.params.Postid;
    const post = await modules.newPostCOll.getManyRelation(
      id,
      modules.like,
      modules.comment
    );
...
async function handleGetAllPostDataWithReplies(req, res, next) {
  try {
    const post = await modules.newPostCOll.getManyRelationWithReplies(
      modules.comment,
      modules.replys,
      modules.like
    );
...
async function handleGetRelation(req, res, next) {
  try {
